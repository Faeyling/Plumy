...
export function QuizCoursSession() {
  const { coursId } = useParams<{ coursId: string }>()
  const navigate = useNavigate()
  const cours = getCours(coursId ?? '')
  const unite = unites.find(u => u.coursIds.includes(coursId ?? ''))

  const [questions, setQuestions] = useState<QuestionQCMCours[]>(() =>
    shuffle(getQuestionsCoursPourCours(coursId ?? ''))
  )
  ...
  function handleRecommencer() {
    setQuestions(shuffle(getQuestionsCoursPourCours(coursId ?? '')))
    setIndex(0)
    setSelected(null)
    setCorrects(0)
    setPtsTotal(0)
      ...
      <div className="flex flex-col min-h-svh">
        <div className="flex-1 flex flex-col items-center justify-center p-8 gap-5 text-center">
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <button
              onClick={() => navigate(`/cours/${coursId}`)}
            ...
            {unite && (
              <button
                onClick={() => navigate(`/unite/${unite.numero}`)}
                className="w-full py-3 text-center bg-[var(--color-candy-lavande-light)] text-[var(--color-encre)] font-[var(--font-titre)] font-semibold rounded-[var(--radius-card)] text-sm"
              >
                {fr.quiz.retourUnite}
              </button>
            )}
